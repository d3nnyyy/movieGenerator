...
import { styled } from '@mui/material/styles';
import axios from 'axios'
import { Checkbox, Slider, FormControlLabel, TextField, Button, LinearProgress, createTheme, ThemeProvider } from "@mui/material";
import { useState } from "react";
...
const App = () => {
  ...
  const handleLengthChange = (event, newValue) => setLength(newValue);
  const handleGenreChange = (genre) => setGenres((prevGenres) => prevGenres.includes(genre) ? prevGenres.filter((g) => g !== genre) : [...prevGenres, genre]);
  ...
  return (
    <div className="App">
      <div className={`${isGenerating ? 'loading' : 'hidden'}`}>
        <h1 className="loader-title">Loading...</h1>
        <LinearProgress color="secondary" />
      </div>
      ...
      <div className={`${(isGenerating || isMovie) ? 'hidden' : ''}`}>
        <h1 className="header">Choose the parameters for your movie:</h1>
  ...
  <div className="checkbox-container-mobile">
      ...
      <div className={`${isMovie ? 'buttons-container' : 'hidden'}`}>
        <ColorButton variant="contained" size="Medium" onClick={submitParams}>Regenerate</ColorButton>
        <ColorButton variant="contained" size="Medium" onClick={() => setIsMovie(false)}>Edit parameters</ColorButton>
      </div>
    </div>
  );
};
